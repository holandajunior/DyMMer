...
public class ExportOfficeExcel {
	...
	private static final String[] METRICS = {"Non-Functional Commonality (NFC)", "Number of features (NF)", "Number of top features (NTop)",
										"Number of leaf Features (NLeaf)", "Depth of tree (DT)", "Cognitive Complexity of a Feature Model (I)", 
										...
										"Cross-tree constraints (CTC)", "Coefﬁcient of connectivity-density (CoC)", "Number of variable features", 
										"Number of variation points", "Single Hotspot Features", "Multiple Hotspot Features", "Rigid Nohotspot Features",
										"Ratio of variability (RoV)", "Number of valid conﬁgurations (NVC)"};
	...
	public void standardExport(String fileName){
		...
		for(int i = 0; i < modelsContext.size(); i++){
			...
			Object dataModel[] = {model.nonFunctionCommonality(), model.numberOfFeatures(), model.numberOfTopFeatures(),
						  model.numberOfLeafFeatures(), model.depthOfTree(), model.cognitiveComplexityOfFeatureModel(),
						  ...
						  model.crossTreeConstraints(), model.coefficientOfConnectivityDensity(), model.numberOfVariableFeatures(),
						  model.numberOfVariationPoints(), model.singleVariationPointsFeatures(), model.multipleVariationPointsFeatures(),
						  model.rigidNotVariationPointsFeatures(), model.productLineTotalVariability(), model.numberOfValidConfigurations(), model.numberOfContexts(), modelName};
		...
		for(int i = 0; i < modelsContext.size(); i++){
			...
			for(Entry<String, Context> contexts : modelContext.getContexts().entrySet()){
				...
				Object dataModelContext[] = {modelContext.nonFunctionCommonality(), modelContext.numberOfFeatures(), modelContext.numberOfTopFeatures(),
						modelContext.numberOfLeafFeatures(), modelContext.depthOfTree(), modelContext.cognitiveComplexityOfFeatureModel(),
						...
						modelContext.crossTreeConstraints(), modelContext.coefficientOfConnectivityDensity(), modelContext.numberOfVariableFeatures(),
